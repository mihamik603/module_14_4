...
def add_sample_products():
    ...
    cursor = conn.cursor()
    sample_products = [("Product1", "Описание 1", 100),
    ...
    conn.commit()
    conn.close()
# add_sample_products()  # Закомментировать после добавления продуктов
...
async def main_menu(message: types.Message):
    inline_keyboard = InlineKeyboardMarkup()
    inline_keyboard.add(InlineKeyboardButton("Рассчитать норму калорий", callback_data='calories'),InlineKeyboardButton("Формулы расчёта", callback_data='formulas'))
